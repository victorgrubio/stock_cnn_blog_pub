...
def train(data_gen):
    while True:
        logger.info("training for batch number {}".format(count))
        ...
        logger.info("Best val_loss is {} and corresponding train_loss is {}".format(
            history.history['val_loss'][min_arg], history.history['loss'][min_arg]))
        test_res = model.evaluate(x_test, y_test, verbose=0)
        logger.info("keras evaluate result =" + str(test_res)+", metrics:"+str(model.metrics_names))
        ...
        logger.info("F1 score (weighted) " + str(f1_weighted))
        logger.info(
        ...
        for i, row in enumerate(conf_mat):
            logger.info("precision of class {} = {}".format(i, np.round(row[i] / np.sum(row), 2)))

        if is_last_batch:
            break

    logger.info("Complete training finished in {}".format(seconds_to_minutes(time.time() - start_time)))
